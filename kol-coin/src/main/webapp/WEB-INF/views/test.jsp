...
<div class="insert_content" id="insert_div">
    <form id="insert_form" class="layui-form" action="/Gift/doInsert" method="post" enctype="multipart/form-data">
        <label class="layui-form-label">礼物名称</label>
        <div class="layui-input-inline">
            <input type="text" name="gift_name" class="layui-input" id="gift_name" lay-verify="required|title">
        </div>
        <label class="layui-form-label">价值</label>
        <div class="layui-input-inline">
            <input type="text" name="value" class="layui-input" id="value" lay-verify="required|number">
        </div>
        <div>
            <%--礼物类别还没确定--%>
            <label class="layui-form-label">礼物类别</label>
            <div class="layui-input-inline" style="width: 100px">
                <select name="gift_type" id="gift_type">
                    <option value="1">小型礼物</option>
                    <option value="2">大型礼物</option>
                </select>
            </div>
        </div>
        <div>
        ...
        <div class="layui-input-inline">
            <input type="text" name="gift_value" class="layui-input" id="gift_value" lay-verify="required|number">
        </div>
        <label class="layui-form-label">魅力值</label>
        <div class="layui-input-inline">
            <input type="text" name="charm_value" class="layui-input" id="charm_value">
        </div>
...
<div class="update_content" id="update_div" style="display: none">
    <form id="update_form" class="layui-form" enctype="multipart/form-data" action="/Gift/editing" method="post">
        <input type="text" name="id" id="update_id" hidden="hidden">

        <label class="layui-form-label">礼物名称</label>
        <div class="layui-input-inline">
            <input type="text" name="gift_name" class="layui-input" id="update_gift_name">
        </div>
        <label class="layui-form-label">价值</label>
        <div class="layui-input-inline">
            <input type="text" name="value" class="layui-input" id="update_value">
        </div>
        <br>
        <label class="layui-form-label">礼物类型</label>
        <div class="layui-input-inline" style="width: 120px">
            <select name="gift_type" id="update_gift_type" >
                <option value="1">小型礼物</option>
                <option value="2">大型礼物</option>
            </select>
        </div>
        ...
        <label class="layui-form-label">礼物值</label>
        <div class="layui-input-inline">
            <input type="text" name="gift_value" class="layui-input" id="update_gift_value">
        </div>
        <br>
        <label class="layui-form-label">状态</label>
        <div class="layui-input-inline" style="width: 120px">
...
<script>
    layui.use(['table','layer','form'], function () {  // 引入 table模块
            ...
            success: function (r) {
                ...
                tableIns =table.render({
                    ...
                    , cols: [[ //表头
                        ...
                        , {field: 'gift_type', title: '礼物类别', align: "center", templet: function (d) {
                                //礼物类别还没确定
                                if (d.gift_type == 1) {
                                    return "小型礼物";
                                } else if (d.gift_type == 2){
                                    return "大型礼物";
                                }
                            }}
                ...
                table.on('tool(gift_table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'on_offline'){ //上下线
                        if (data.state==1){//下线操作
                            ...
                            $.ajax({
                                ...
                                data:{id:id},
                                success: function (r) {
                                    window.location.href="/Gift/showPage";
                                },
                                error:function () {
                                    alert("失败")
                                }
                    ...
                    //编辑操作
                        ...
                        $("#update_gift_type").val(data.gift_type);
                ...
                $('#deleteButton').on('click', function() {
                    ...
                    layer.confirm('确定删除id为:'+ids+"这"+data.length+"条数据吗？", {
                        ...
                        window.location.href="/Gift/delete?id="+ids;
                    }, function(index){
                    });
                })
            }
